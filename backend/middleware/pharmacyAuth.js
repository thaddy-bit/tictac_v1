...
/**
 * Middleware pour authentifier les pharmaciens
 */
const pharmacyAuth = async (req, res, next) => {
  try {
    ...
    // Vérifier si l'utilisateur est une pharmacie ou un admin
    if (decoded.role !== 'pharmacy' && decoded.role !== 'admin') {
      return res.status(403).json({ error: 'Accès refusé : Droits pharmacien requis' });
    }
